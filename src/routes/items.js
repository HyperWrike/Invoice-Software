/**
 * Item/product management routes (async / Postgres).
 */
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const r = await db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(
            req.business.id, name.trim(), description || null,
        ...
        const row = await db.prepare('SELECT * FROM items WHERE id = ?').get(r.lastInsertRowid);
        res.json(row);
    } catch (err) { next(err); }
});
