...
router.get('/summary', (req, res) => {
  ...
  const row = (sql, ...params) => db.prepare(sql).get(bid, ...params) || {};
  const all = (sql, ...params) => db.prepare(sql).all(bid, ...params) || [];

  const totalInvoices = row('SELECT COUNT(*) AS c FROM invoices WHERE business_id = ?').c || 0;
  const totalRevenue = row(`SELECT COALESCE(SUM(total),0) AS s FROM invoices WHERE business_id = ? AND status = 'paid'`).s || 0;
  const outstanding = row(`SELECT COALESCE(SUM(total),0) AS s FROM invoices WHERE business_id = ? AND status IN ('sent','overdue','draft')`).s || 0;
  const gstCollected = row(`SELECT COALESCE(SUM(gst_total),0) AS s FROM invoices WHERE business_id = ? AND status = 'paid'`).s || 0;
...
// GET /api/reports/gst - GST report per rate for a date range
...
router.get('/audit', (req, res) => {
  ...
  `).all(req.business.id);
  rows.forEach(r => { try { r.details = r.details ? JSON.parse(r.details) : null; } catch(_){} });
  res.json(rows);
});
